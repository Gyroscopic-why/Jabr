...
namespace JabrAPI
{
    static internal partial class Miscellaneous
    {
        ...
        {
            bool isValid = IsMessageValid(message, out exception);
            if (isValid) isValid = IsReKeyValid(reKey, out exception);
            return isValid;
        }
        static internal bool IsMessageAndReKeyValid(
            string message, IEncryptionKey reKey, bool throwExceptions)
        {
            bool isValid = IsMessageAndReKeyValid(message, reKey, out Exception? exception);
            if (!isValid && throwExceptions) throw exception!;
            return isValid;
        }
