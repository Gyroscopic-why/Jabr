...
namespace JabrAPI
{
    ...
    {
        static private List<MiniInterval> ApplyFilter_DIFFERENCE_TO_MAX(
                ...
                case IntervalFilters.FilterSelectionState.MIN:
                    {
                        ...
                        {
                            if (interval.DifToMax <= paramCountingRef.LowDifToMax)
                            {
                                result.Add(interval);
                ...
                case IntervalFilters.FilterSelectionState.MAX:
                    {
                        foreach (MiniInterval interval in valid)
                        {
                            if (interval.DifToMax >= paramCountingRef.HighDifToMax)
                            {
                                result.Add(interval);
                                paramCountingOut.Update(interval);
                                allChanceOut += interval.Probability;
