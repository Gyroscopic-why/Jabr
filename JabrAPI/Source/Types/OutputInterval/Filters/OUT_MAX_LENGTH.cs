...
namespace JabrAPI
{
    ...
    {
        static private List<MiniInterval> ApplyFilter_MAX_OUT_LENGTH(
                ...
                case IntervalFilters.FilterSelectionState.MIN:
                    {
                        ...
                        {
                            if (interval.MaxVal <= paramCountingRef.LowMaxVal)
                            {
                                result.Add(interval);
                ...
                case IntervalFilters.FilterSelectionState.MAX:
                    {
                        foreach (MiniInterval interval in valid)
                        {
                            if (interval.MaxVal >= paramCountingRef.HighMaxVal)
                            {
                                result.Add(interval);
                                paramCountingOut.Update(interval);
                                allChanceOut += interval.Probability;
