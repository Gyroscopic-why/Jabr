...
namespace JabrAPI
{
    ...
    {
        static private List<MiniInterval> ApplyFilter_ABSOLUTE_DIFFERENCE(
                ...
                case IntervalFilters.FilterSelectionState.MIN:
                    {
                        ...
                        {
                            if (interval.AbsDif <= paramCountingRef.LowAbsDif)
                            {
                                result.Add(interval);
                ...
                case IntervalFilters.FilterSelectionState.MAX:
                    {
                        foreach (MiniInterval interval in valid)
                        {
                            if (interval.AbsDif >= paramCountingRef.HighAbsDif)
                            {
                                result.Add(interval);
                                paramCountingOut.Update(interval);
                                allChanceOut += interval.Probability;
